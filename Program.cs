...
namespace Training {
   internal class Program {
      static void Main (string[] args) {
         ...
         ComplexNumber output = ComplexNumber.Add (input1, input2);
   ...
   class ComplexNumber {
      ...
      public ComplexNumber ComplexNum (string input) {
         ...
         char SignI = imaginary.ToString ().Contains ('-') ? '-' : '+';
         ComplexNumber n = new () {
            realPart = RealPart,
      ...
      public override string ToString () {
         return this.realPart.ToString () +this.sign+ this.imaginaryPart.ToString () + "i";
      }
